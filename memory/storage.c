...
MEM_Storage
MEM_open_storage_func(char *filename, int line, int page_size)
{
    MEM_Storage	storage;

    storage = MEM_malloc_func(filename, line,
...
void*
MEM_storage_malloc_func(char *filename, int line, MEM_Storage storage,
			size_t size)
{
    int			cell_num;
    MemoryPage		*new_page;
	...
	new_page = MEM_malloc_func(filename, line,
				   sizeof(MemoryPage)
...
void
MEM_dispose_storage(MEM_Storage storage)
{
    MemoryPage	*temp;
	...
	memset(storage->page_list->cell, 0xCC,
	       storage->page_list->cell_num * sizeof(Cell));
